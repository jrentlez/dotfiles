...
impl Section {
    ...
    fn last_line(writer: &mut impl Write, job_count: usize, last_status: u8, shell: Shell) {
        colored_prompt_character(writer, job_count, last_status, shell);
        write_bytes!(writer, shell.fg_normal(), b" ");
    }

    pub fn write(self, mut writer: impl Write, shell: Shell, job_count: usize, last_status: u8) {
        match self {
            Self::PreCmd => Self::pre_cmd(&mut writer, shell),
            Self::LastLine => {
                Self::last_line(&mut writer, job_count, last_status, shell);
            }
            Self::All => {
